...
export function covertSVG2Image(node, name, width, height, type = 'png') {
...
export default function multiavatar() {
  ...
  const final = ['env', 'clo', 'head', 'mouth', 'eyes', 'top'].reduce((p, n) => {
    p[n] = getFinal(n);
    return p;
  }, {});

  function getFinal(part) {
    const partV = `0${Math.floor(Math.random() * 47)}`.substring(0, 2);
    const theme = ['A', 'B', 'C'][Math.floor(Math.random() * 3)];
